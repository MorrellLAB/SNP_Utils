...
try:
    ...
    sys.exit(
        "Please make sure you are in the 'SNP_Utils' directory to load custom modules:"
        + error.name
    )
...
def blast_based(args, lookup_dict):
    ...
    for query in blast_soup.findAll("Iteration"):
        ...
        if (
            blast.get_value(tag=query, value="Iteration_message").capitalize()
            == blast.NO_HIT_MESSAGE
        ):
            ...
            for hit in query.findAll("Hit"):
                ...
                this_hsps = blast.parse_hit(
                    snpid=snpid, hit=hit
                )  # Parse the HSPs from this Hit
...
def write_outputs(args, snp_filter, masked_filter, no_snps, ref_gen, vcf_info):
    ...
    if len(masked_list) > 0:
        print(
            "Writing", len(masked_list), "masked SNPs to", maskedfile, file=sys.stderr
        )
...
def main():
    ...
    if args["method"] == "CONFIG":
        ...
        if args["method"] == "BLAST":
            ...
            vcf_info.append(
                INFO(
                    ...
                    number=0,
                    infotype="Flag",
                    description="Variant Calculated from BLAST",
                )
            )
            ...
            vcf_info.append(
                INFO(
                    ...
                    number=0,
                    infotype="Flag",
                    description="Variant Calculated from SAM",
                )
            )
        #   Start our filtering
        ...
        if "map" in args and args["bychrom"]:
            proper_snps, vcf_info = filters.chrom_filter(
                args=args, vcfinfo=vcf_info, propersnps=proper_snps
            )
        if "threshold" in args.keys():
            proper_snps, vcf_info = filters.distance_filter(
                args=args, vcfinfo=vcf_info, propersnps=proper_snps
            )
        if "map" in args and args["bydistance"] and ref_gen is not REFERENCE_DEFAULT:
            proper_snps, vcf_info = filters.map_filter(
                args=args,
                vcfinfo=vcf_info,
                propersnps=proper_snps,
                refgen=ref_gen,
                method=method,
                bconf=bconf,
            )
            ...
            print(
                "Could not determine reference genome, not filtering by genetic map distance",
                file=sys.stderr,
            )
