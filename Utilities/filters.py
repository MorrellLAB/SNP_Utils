...
ALTPOS = INFO(  # Create an info object for alternate positions
    infoid="ALTPOS", number=".", infotype="Integer", description="Alternate positions"
)
...
def distance_filter(args, vcfinfo, propersnps):
    ...
    try:  # Value checking
        ...
        raise ValueError
    print(
        "Filtering SNPs with a minimum distance threshold of",
        args["threshold"],
        file=sys.stderr,
    )
    #   Add our VCF header
    ...
    for snplist in snp_dict.values():
        ...
        while (
            sorted_snps
        ):  # The while loop stops when we're done with our list of sorted SNPs
            left_index = len(left_snps) - 1  # Take the furthest right of our left SNPs
            next_snp = sorted_snps.pop(0)  # Get the next SNP
            if (
                next_snp - left_snps[left_index] < args["threshold"]
            ):  # If the distance between the two SNPs are less than our minimum distance threshold
                #   Add the next SNP as an alternate position to our left SNP
                left_snps[left_index].add_info(
                    key=ALTCHR.get_id(), value=next_snp.get_chrom()
                )
                left_snps[left_index].add_info(
                    key=ALTPOS.get_id(), value=next_snp.get_position()
                )
            else:  # Otherwise, add the next SNP to our list of left-most SNPs
                left_snps.append(next_snp)
        filtered_snps += (
            left_snps  # Add our list of left-most SNPs to the overall SNP list
        )
    ...
    for key, snplist in snp_dict.items():
        ...
        try:
            ...
            closest = closest_value(
                iterable=length_dict, value=map_proportion, return_item=True
            )
            these_snps.remove(closest)
            for remaining in these_snps:
                closest.add_info(key=ALTCHR.get_id(), value=remaining.get_chrom())
                closest.add_info(key=ALTPOS.get_id(), value=remaining.get_position())
            closest_snps.append(closest)
        except (
            AssertionError
        ):  # Don't need to do all this crap and spit out error messages
